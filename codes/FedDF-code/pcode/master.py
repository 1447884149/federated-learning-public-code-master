...
class Master(object):
    ...
    def run(self):
        for comm_round in range(1, 1 + self.conf.n_comm_rounds):
            ...
            # get random n_local_epochs.本地训练轮数列表
...
# 用于聚合模型并在测试数据上进行评估
    def _aggregate_model_and_evaluate(self, flatten_local_models):
        ...
        # 如果flatten_local_models中都是一个架构，则自然只需要取第一个聚合模型，否则为none
        # _avg_over_archs返回的是字典{架构：聚合模型}
        ...
        # aggregate_fn is not None表示聚合策略不为federated_average
        if self.aggregator.aggregate_fn is not None:
            # evaluate the uniformly averaged model.

            if fedavg_model is not None:# 只有一种架构
                # 计算在多个数据加载器上的平均性能
            ...
            # 根据聚合器的聚合函数和条件来进一步聚合模型。如果聚合函数存在，则将根据聚合函数进行模型聚合。这是为了根据特定的聚合策略和条件，将不同架构的聚合模型和本地模型进行进一步的聚合操作。是为了满足更高级的聚合策略和需求，以便更好地整合和更新模型。
            client_models = self.aggregator.aggregate(
                master_model=self.master_model,
                client_models=self.client_models,
                fedavg_model=fedavg_model,
                fedavg_models=fedavg_models,# 字典{架构：聚合模型}
                flatten_local_models=flatten_local_models,
                performance=performance,
            )
            # 在相同模型架构的情况下，client_models 和 self.master_model 将被就地更新。
